...
date: '2015-02-07'
bibliography: [karyl.bib]
subject: k-ary, n-ary, hierarchies, process events, probability, R-code
description: probability distribution of k-ary tree levels, with application for hierarchical
  processes.
---

```{r setup, echo=FALSE, purl=FALSE,message=FALSE,warning=FALSE}
knitr::opts_chunk$set(comment="",message=FALSE,warning=FALSE,highlight=TRUE,echo=TRUE)
library(diagram,warn.conflicts=FALSE,quietly=TRUE)
```
...
<figure>
<center>
```{r simple_tree, echo=FALSE, fig.width=4, fig.height=4, fig.retina=2, purl=FALSE}
L <- 3  # number of levels
g <- 3  # group size (number of lower level nodes in a group)
NperLevel <- g^(0:(L-1))
N <- sum(NperLevel)  # total number of elements

# matrix of node connections
H <- matrix(nrow=N, ncol=N, data=0)
for(i in seq(2,N,g)) H[i:(i+g-1),(i-2)/g+1] <- 1

# plot using diagram package
plotmat(t(H), pos=NperLevel, name=rep("",N), curve=0,
		box.type="circle", box.size=0.035, box.col="black", shadow.size=0,
		arr.pos=1, arr.type="circle", arr.col="white", cex.txt=0)
```
...
Using the *diagram* package [@Soetaert2009], I have drawn a [Hasse](http://en.wikipedia.org/wiki/Hasse_diagram) diagram in *Figure 1* of a small example hierarchy of `r sum(NperLevel)` possible events, some of which are higher level, which means they affect multiple lower-level transactions. In this example, it's easy to see that there are 4 possible events where more than a single transaction is affected. This type of hierarchy is called an [k-ary tree](http://en.wikipedia.org/wiki/K-ary_tree). For the purpose of estimating the number of transactions affected by a higher level event, we can turn the tree into a a probability function.
